...
<script>
	...
	$(document).ready(function(){
		...
		$("#btnCancel").on("click", function(){
			...
			location.href = "/diary/diaryView?bno=${update.bno}"
				   + "&page=${scri.page}"
		...
		$("#btnSave").on("click", function(){
			formObj.attr("action", "/diary/diaryUpdate?bno="+${update.bno});
	...
	$(document).on('click', '#btnList', function(){
		location.href="/diary/diaryList?bno={update.bno}"
				+ "&page=${scri.page}"
			    + "&perPageNum=${scri.perPageNum}"
			    + "&searchType=${scri.searchType}"
			    + "&keyword=${scri.keyword}";
...
<header id="header">
	<%@ include file="../include/header.jsp" %>
		...
		<div class="container" role="main">
			<form id="form" name="updateForm" method="post" action="/diary/diaryUpdate" enctype="multipart/form-data">
...
<script>
CKEDITOR.replace('content',{filebrowserUploadUrl:'/diary/imageUpload'});
</script>	
</body>
</html>
