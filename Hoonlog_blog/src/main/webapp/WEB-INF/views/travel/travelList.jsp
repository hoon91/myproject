...
<script>
	...
	$(document).on('click', '#btnWriteForm', function(e){
		e.preventDefault();
		location.href = "/travel/travelWrite";
	});
	
	
	/* 글상세보기 이벤트*/
	function fn_contentView(bno){
		var url = "/travel/travelView";
		url = url + "?bno="+bno;
		location.href = url;
	}
...
<header class="header">
	...
	<div class="container">
			<h1 align="center" style="margin-bottom: 30px;">TRAVEL</h1>
		<hr>
				...
				<c:choose>
					...
					<c:forEach items="${travelList}" var = "list">
						<tr>
							...
							<td>
								<a href="/travel/travelView?bno=${list.bno}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}">
	...
	<div align="center">
  			<ul class="pagination justify-content-center">
    			...
    			<li class="page-item"><a href="/travel/travelList${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
   				</c:if> 

    			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    			<li class="page-item"><a href="/travel/travelList${pageMaker.makeSearch(idx)}">${idx}</a></li>
    			</c:forEach>

    			<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
    			<li class="page-item"><a href="/travel/travelList${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
    			</c:if> 
  			</ul>
	</div>
