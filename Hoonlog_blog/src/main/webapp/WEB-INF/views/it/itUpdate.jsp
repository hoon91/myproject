...
<script>
	...
	$(document).ready(function(){
		...
		$("#btnCancel").on("click", function(){
			...
			location.href = "/it/itView?bno=${update.bno}"
				   + "&page=${scri.page}"
		...
		$("#btnSave").on("click", function(){
			formObj.attr("action", "/it/itUpdate?bno="+${update.bno});
	...
	$(document).on('click', '#btnList', function(){
		location.href="/it/itList?bno={update.bno}"
				+ "&page=${scri.page}"
			    + "&perPageNum=${scri.perPageNum}"
			    + "&searchType=${scri.searchType}"
			    + "&keyword=${scri.keyword}";
...
<header id="header">
	<%@ include file="../include/header.jsp" %>
		...
		<div class="container" role="main">
			<form id="form" name="updateForm" method="post" action="/it/itUpdate" enctype="multipart/form-data">
...
<script>
CKEDITOR.replace('content',{filebrowserUploadUrl:'/it/imageUpload'});
</script>	
</body>
</html>
