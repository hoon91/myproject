...
<script>
	...
	$(document).ready(function(){
		...
		$("#btnCancel").on("click", function(){
			...
			location.href = "/board/boardView?bno=${update.bno}"
				   + "&page=${scri.page}"
		...
		$("#btnSave").on("click", function(){
			formObj.attr("action", "/board/boardUpdate?bno="+${update.bno});
	...
	$(document).on('click', '#btnList', function(){
		location.href="/board/boardList?bno={update.bno}"
				+ "&page=${scri.page}"
			    + "&perPageNum=${scri.perPageNum}"
			    + "&searchType=${scri.searchType}"
			    + "&keyword=${scri.keyword}";
...
<header id="header">
	<%@ include file="../include/header.jsp" %>
		...
		<div class="container" role="main">
			<form id="form" name="updateForm" method="post" action="/board/boardUpdate" enctype="multipart/form-data">
...
<script>
CKEDITOR.replace('content',{filebrowserUploadUrl:'/board/imageUpload'});
</script>	
</body>
</html>
